...
def usage():
	print 'archivetool.py cmd <options> directory'
	...
	print '        default recovery percent 5, volume size 4g'
	print '    listnonrar: find all non rar files'

def main(argv) :
	global file_stdout, file_stderr, file_status
	fun_hash = {'test' : test_rar, 'lock' : lock_rar, 'unlock': unlock_rar, 'addrecover' : addrecover_rar, 'create': create_rar, 'listnonrar': list_nonrar}
	options_hash = {'volume_size':'4g', 'force':False, 'recovery_percent':5}
	try:
		...
		if len(args) != 1 or not fun_hash.has_key(argv[1]):
			usage()
			sys.exit(1)
		else:
			directory = args[0]
